...
zodiac_file_location = None
with open("zodiac_file_location.txt") as f:
    zodiac_file_location = f.readline().strip()
zodiac_names: Dict[str, str] = dict()
"""{ person: date }"""
...
def main():
    ...
    pd = pandas.read_excel(zodiac_file_location)
    for idx, row in pd.iterrows():
        zodiac_names[row["Name"]] = dict()
        zodiac_names[row["Name"]]["day"] = row["Day"]
        zodiac_names[row["Name"]]["month"] = list(calendar.month_name).index(row["Month"])  # Convert month name to int
        zodiac_names[row["Name"]]["year"] = row["Year"]
    ...
    print("Total contacts analyzed: " + str(contacts_analyzed))
    print("Total contacts updated: " + str(contacts_updated))
...
def do_stuff_to_connections_list(service, connectionsList: list):
    ...
    for person in connectionsList:
        ...
        if birthdays and names:
            try:
                logging.info("NAME: " + name + "\t\tBirthday YEAR: " + str(birthday["year"]))
            ...
            if name in zodiac_names.keys():
                logging.debug("Adding birthday (" + str(zodiac_names[name]["month"]) + "/" + str(
                    zodiac_names[name]["day"]) + "/" + str(zodiac_names[name]["year"]) + ") for " + name)
                ...
                birthday_obj["text"] = str(zodiac_names[name]["month"]) + "/" + str(
                    zodiac_names[name]["day"]) + "/" + str(zodiac_names[name]["year"])
                birthday_obj["date"]["year"] = zodiac_names[name]["year"]
                birthday_obj["date"]["month"] = zodiac_names[name]["month"]
                birthday_obj["date"]["day"] = zodiac_names[name]["day"]
